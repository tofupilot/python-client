...
def withTofuPilot(
    ...
    """
    Convenience function to run an OpenHTF test with TofuPilot integration.
    
    This function provides a simple way to execute OpenHTF tests with automatic
    TofuPilot integration for real-time monitoring and result upload.
    
    Args:
        test: The OpenHTF test instance to execute
        test_start_callable: Function that returns the DUT serial number
        stream: Whether to enable real-time streaming to TofuPilot Operator UI
        api_key: Optional API key (if not provided, uses environment variable)
        url: Optional TofuPilot URL (if not provided, uses default)
        
    Returns:
        The test record from the executed test
        
    Example:
        ```python
        from openhtf import Test
        from tofupilot.openhtf import withTofuPilot
        
        test = Test(*phases, procedure_id="FVT1")
        
        # Execute test with TofuPilot integration
        test_record = withTofuPilot(
            test=test,
            test_start_callable=lambda: "SN12345",
            stream=True
        )
        ```
    """
    
    # Add upload callback to automatically upload results
    test.add_output_callbacks(upload(api_key=api_key, url=url))
    
    # Execute test with TofuPilot context manager for streaming
    with TofuPilot(test, stream=stream, api_key=api_key, url=url):
        test_record = test.execute(test_start_callable)
    
    return test_record


def execute_with_graceful_exit(test, test_start):
    """
    Helper function for more reliable Ctrl+C handling in OpenHTF tests.
    
    This function provides better handling of KeyboardInterrupt exceptions
    during test execution, ensuring proper cleanup of TofuPilot resources.
    
    Args:
        test: The OpenHTF test instance to execute
        test_start: Function that returns the DUT serial number
    ...
    try:
        return test.execute(test_start)
