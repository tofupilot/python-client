...
from tofupilot.v2.models.part_deleteop import PartDeleteResponse
...
def assert_delete_part_success(result: PartDeleteResponse) -> None:
    """Assert that part delete response is valid."""
    assert checkcast(PartDeleteResponse, result)
    assert len(result.id) > 0
